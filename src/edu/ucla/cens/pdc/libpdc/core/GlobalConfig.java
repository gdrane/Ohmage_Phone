...
public class GlobalConfig implements iState {
	...
	public final synchronized void addApplicationOnPhone(android.app.Application app) {
		assert _root != null : "Root wasn't set";
		...
		if(ai == null) {
		  throw new Error("Please provide a name to application");
		}
		_applications_on_phone.put((String)pm.getApplicationLabel(ai), app);
	}
	/**
	...
	public synchronized android.app.Application getApplicationOnPhone(
			String name) {
			assert _root != null : "Root wasn't set";
			return _applications_on_phone.get(name);
	}
...
// </editor-fold>
	...
	private transient final Map<String, Application> _applications = 
			new HashMap<String, Application>();
	
	private transient final Map<String, android.app.Application> 
	_applications_on_phone = new HashMap<String, android.app.Application>();
